...
function project2welve_preprocess_page(&$vars) {
	 ...
	 //Hide page Title in issues Edit form
	 if(arg(0) == 'node' && arg(2) == 'edit' && is_numeric(arg(1))){
	    $node = node_load(arg(1));
		if(in_array($node->type, array('issues', 'issues_from_app')))
		  $vars['title'] = '';
	 }
...
function project2welve_preprocess_html(&$vars) {
  ...
  //Add Publisher Tag
	$element1 = array(
	  '#tag' => 'link',
	  '#attributes' => array( 
		'href' => 'https://plus.google.com/114393702804641685337', 
		'rel' => 'publisher',
	  ),
	);
	$element2 = array(
	  '#tag' => 'base',
	  '#attributes' => array( 
		'href' => 'http://www.iamin.in', 
	  ),
	);
	drupal_add_html_head($element1, 'publisher_path');
    drupal_add_html_head($element2, 'site_path');
	switch(arg(0))
        {
		case 'user':
